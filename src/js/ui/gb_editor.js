...
class GridEditor {
    ...
    this.SHIPS.forEach((blockInfo, index) => {
      ...
      button.addEventListener("click", () => {
        if (
          ...
          let selectedButton = shipList.querySelector(
            `[data-shipindex="${this.currentShipIndex}"]`,
          );
  ...
  #addInputEventListeners() {
    ...
    this.startButton.addEventListener("click", () => {
      ...
      for (let cell of allGridCells) {
        if (
          cell.getAttribute("data-start") === "true" &&
          cell.getAttribute("data-orientation") &&
          cell.getAttribute("data-length")
        ) {
          let startingCoords = [
          ...
          this.#internalGameboard.placeShip(
            newShip,
            startingCoords,
            orientation,
          );
  ...
  #blockPreview() {
    this.grid.addEventListener("mouseleave", () => {
      for (let i = 0; i < this.grid.children.length; i++) {
        if (
          this.grid.children[i].classList.contains("hovered") ||
          this.grid.children[i].classList.contains("hovered-error")
        ) {
    ...
    this.grid.addEventListener("mouseover", (event) => {
      ...
      for (let i = 1; i <= difference; i++) {
        ...
        if (this.currentOrientation == "h") {
          ...
          const next_row =
            Number(event.target.getAttribute("data-row-index")) + i;
  ...
  #addBlockPlacement() {
    ...
    grid.addEventListener("click", (event) => {
      ...
      if (
        !event.target.classList.contains("hovered-error") &&
        !this.#isAllPlaced()
      ) {
        ...
        const targetShip = shipList.querySelector(
          `[data-shipindex="${this.currentShipIndex}"]`,
        );
  ...
  #goToNextBlock() {
    ...
    if (
      !this.SHIPS[this.currentShipIndex + 1] ||
      this.SHIPS[this.currentShipIndex + 1].placed
    ) {
